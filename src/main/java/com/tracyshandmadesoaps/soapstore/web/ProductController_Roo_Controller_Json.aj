...
import com.tracyshandmadesoaps.soapstore.web.ProductController;
...
privileged aspect ProductController_Roo_Controller_Json {
    ...
    @RequestMapping(params = "find=ByLineEquals", headers = "Accept=application/json")
    @ResponseBody
    public ResponseEntity<String> ProductController.jsonFindProductsByLineEquals(@RequestParam("line") Line line) {
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json; charset=utf-8");
        return new ResponseEntity<String>(Product.toJsonArray(Product.findProductsByLineEquals(line).getResultList()), headers, HttpStatus.OK);
    }
